...
single-threaded daemon. The subprocess `timeout` is the anti-wedge backstop: `inject`
runs inside the daemon's `serve()` accept loop, so a stuck `wtype` (compositor stall)
...
def inject(text: str, *, timeout_s: float = _TIMEOUT_S) -> None:
    """Type `text` into the focused window via wtype. No-op on empty/whitespace.
    Never raises: a non-zero wtype return, a missing binary, or a spawn that
    overruns `timeout_s` is surfaced on stderr instead of crashing or wedging the
    daemon."""
    if not text.strip():
        return
    try:
