...
sync hint that makes the two toasts share one slot, and the `--` that keeps user
text out of the option parser), the persistent-vs-fading timeouts, the
empty-transcript fallback, the two failure toasts (wtype didn't type it; the
language code was refused), and the never-raise contract (a missing binary, a
non-zero return, or a hung spawn is reported on stderr, never surfaced to the
daemon loop).
"""
...
def _patch_run(**kwargs):
    """Patch `notify-send` with a *successful* spawn. Without an explicit rc the
    MagicMock's `returncode` is truthy, which the new non-zero check would report."""
    kwargs.setdefault("return_value", subprocess.CompletedProcess([], 0, "", ""))
    return mock.patch("susurro.notify.subprocess.run", **kwargs)
...
# --- user text never reaches the option parser ------------------------------
#
# The body is a *transcript*. `notify-send … "✓ Done" "-5 degrees outside"` exits 1
# with `Unknown option -5 degrees outside` and posts nothing — and posting nothing
# means the persistent `-t 0` recording toast is never replaced, so it hangs on
# screen forever. Whisper emits dialogue dashes routinely in pt, and this is the
# only path where a failed-injection transcript survives at all.


@pytest.mark.parametrize(
    ("toast", "expected_body"),
    [
        (lambda n: n.done("-5 degrees outside"), "-5 degrees outside"),
        (lambda n: n.done("- Olá, tudo bem?", injected=False), "- Olá, tudo bem?"),
    ],
    ids=["done", "not-typed"],
)
def test_a_leading_dash_transcript_is_passed_as_text(toast, expected_body):
    with _patch_run() as run:
        toast(Notifier())
    cmd = _cmd(run)
    assert "--" in cmd  # ends option parsing (same guard as inject.py)
    assert cmd[-1] == expected_body
    assert cmd.index("--") < cmd.index(expected_body)  # summary + body both after it
    # Nothing option-shaped is left for GLib to choke on after the terminator.
    assert all(not arg.startswith("-") for arg in cmd[cmd.index("--") + 1 : -1])


def test_a_nonzero_notify_send_is_reported(capsys):
    # It used to be silent: `_send` only caught exceptions, so a toast that never
    # posted looked exactly like one that did.
    with _patch_run(return_value=subprocess.CompletedProcess([], 1, "", "Unknown option -5")):
        Notifier().done("-5 degrees outside")  # must not raise either
    err = capsys.readouterr().err
    assert "notify-send failed (rc=1)" in err
    assert "Unknown option -5" in err  # notify-send's own complaint, not a guess
...
def test_hung_notify_send_is_swallowed(capsys):
    with _patch_run(side_effect=subprocess.TimeoutExpired("notify-send", 5.0)):
